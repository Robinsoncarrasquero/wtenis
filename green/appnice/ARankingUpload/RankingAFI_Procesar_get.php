...
//Programa de Prueba al procesar los archivos

//sleep(1);
$disciplina_Filtro='TDC';//$_GET['d']; 
$fecha_rk_filtro= "2019-06-19";//$_GET['f']; 
...
$rsFileRank= Rank::FileRanking($disciplina_Filtro,$categoria_Filtro,$fecha_rk_filtro);
$jdata=array();


//Seleccionamos la disciplina y la categoria
if ($disciplina_Filtro=='TDC'){
    if ($categoria_Filtro=="PN" || $categoria_Filtro=='PV'){
       rkpac($rsFileRank,$jdata);
    }  else {
...
if (count($jdata)>0){
    $jsondata = array("Success" => TRUE,"Mensaje"=>"Registros Procesados :".count($jdata)); 
}else{
...
function rkjunior($rsFileRank,&$jdata){
   ...
   $jj=0;
    foreach ($rsFileRank as $rk_record) {
        ...
        if ($jj==0){
            //$fechark_delete='2019-01-01';
            //Ranking::DeleteCategoria($disciplina,$categoria,$fechark,$sexo);
        ...
        foreach ($files_procesar as $file_name){      
            if (file_exists($file_name)){
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    //Internacionales
                    RankingDetalle::ranking_detalle("IM",$sheet->getCell("CV" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("IMD",$sheet->getCell("CW" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("COSAT",$sheet->getCell("DA" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("COTTEC",$sheet->getCell("DB" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("1ITFN",$sheet->getCell("DE" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("1ITFND",$sheet->getCell("DF" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("2ITFN",$sheet->getCell("DG" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("2ITFND",$sheet->getCell("DK" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("ITF",$sheet->getCell("DM" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("ATPWTA",$sheet->getCell("DC" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("TPI",$sheet->getCell("DQ" . $row)->getValue(), $array_data);
                    //Nacionales
                    ...
                    //otros
                    RankingDetalle::ranking_detalle("NE",$sheet->getCell("CX" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("IR",$sheet->getCell("CY" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("IRD",$sheet->getCell("CZ" . $row)->getValue(), $array_data);
                    
                     //Penalidad
                    RankingDetalle::ranking_detalle("PENA",$sheet->getCell("N" . $rowx)->getValue(), $array_data);
                    
                    //Totales
                    RankingDetalle::ranking_detalle("TMA",$sheet->getCell("DS" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("TTS",$sheet->getCell("DU" . $row)->getValue(), $array_data);
                    RankingDetalle::ranking_detalle("TTD",$sheet->getCell("DW" . $row)->getValue(), $array_data);
                    
                   
                    //$TTP=($sheet->getCell("DG" . $row)->getValue());
                    $TTP=implode("",explode(".",($sheet->getCell("DY" . $row)->getValue())));                
                    $TTPUNTOS = $TTP;
...
function rkpac($rsFileRank,&$jdata) {
    $jj=0;
    foreach ($rsFileRank as $rk_record) {
        $disciplina=$rk_record['disciplina'];
        $categoria=$rk_record['categoria'];
