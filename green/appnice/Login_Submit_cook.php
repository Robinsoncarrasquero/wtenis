...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $chkcookie = $_POST['cook'];
    $cookie_name="loginID";
    if(!isset($_COOKIE[$cookie_name]) && $chkcookie) {
        $cookie_value_bytes = mt_rand(0,999999);
        //$cookie_value_bytes = sha1(strval(rand(0, microtime(true)) . $usuario . strval(microtime(true))));

        $cookie_new=TRUE;
   } else {
        $cookie_value_bytes=$_COOKIE[$cookie_name];
        $cookie_new=FALSE;
    ...
    $sql = "SELECT cookie,atleta_id,estado,cedula,contrasena,nombres,apellidos,niveluser,email,clave_default FROM atleta WHERE cedula='".$usuario."' && bloqueado=0";
   ...
   if (mysql_num_rows($result)){
        ...
        foreach($array_usuarios as $item){
           ...
           if($usuario === $item['usuario'] 
                   ...
                   || $cookie_value ===$cookie_value_bytes)){
        ...
        //Crea la cookie por primera vez
//        if ($cookie_new && $usuario_encontrado){
//            if (cookie_update($usuario, $cookie_value_bytes)){
//                setcookie($cookie_name,$cookie_value_bytes, time() + 3600, '/');
//            }
//        }
...
//Funciona que cree la cookie
function cookie_update($usuario,$cookie_value){
    
    if ($_POST['cook']){
        $sql = "UPDATE atleta set cookie=".$cookie_value. " WHERE cedula='".$usuario."' && bloqueado=0";
        $result = mysql_query($sql);
        if (!$result){
