    ...
    if (mysqli_num_rows($result)){
        if ($record['email'] !=null){
            ...
            if($sql>0) {
                ...
                $error_login=true;
                //echo "0";
                $jsondata= array("Mensaje"=>TRUE,"Mensaje"=>"La clave fue Reestablecida y Enviada a su Correo ".$record['email'],'HTML'=>"");
            }else{
               $mensaje="Error: No se pudo recuperar la contraseña reintente nuevamente!!";
               $error_login=true;
               //echo "1";
               $jsondata= array("Mensaje"=>FALSE,"Mensaje"=>"Error: No se pudo recuperar la contraseña intente nuevamente!!",'HTML'=>"");  
            }
        }else{
            ...
            if($afechanac[2] !=$bfechanac[2] || $afechanac[1]!=$bfechanac[1] || $afechanac[0]!=$bfechanac[0]){
              ...
              // echo "1";
               $jsondata= array("Success"=>FALSE,"Mensaje"=>"Error: La fecha de nacimiento esta Errada",'HTML'=>"");  
                ...
                $nueva_clave = substr(md5($time),1,6); // encriptamos la nueva contraseña con md5
                $sql = mysqli_query($conn,"UPDATE atleta SET contrasena='".$cedula."' ,clave_default=NULL  WHERE cedula='".$cedula."'");
                if($sql>0) {
                    email_notificacion("RecuperarClave", $cedula);
