...
$torneo_id=  Encrypter::decrypt($_GET['torneo_id']);

$atleta_id=$_SESSION['atleta_id']; // Atletaid
$codigo_seguridad=Encrypter::encrypt($torneo_id.$atleta_id);
...
//print_r($id);
    ...
    $obj->Fetch('FVT');
    $lacarta=$obj->getConstancia(); // La constancia
    ...
    if ($objMisTorneos->Operacion_Exitosa()){
        ...
        $buscar = array("@@CEDULA", "@@ATLETA", "@@NOMBRE", "@@TORNEO","@@FECHA_DESDE","@@FECHA_HASTA","@@DIAS","@@MES","@@ANO","@@ESTADO");
        $mudar   = array($cedula, $atleta_titulo,'USTED NO ASISTIO EN ESTE TORNEO'.$torneo_id, $fecha_desde,$fecha_hasta,$dias, fun_Mes_Literal($mes),$ano,$mi_estado);
